...
#define PR_FIXED_FMT "%d.%d"
#define FIXED_TO_FLOAT(fixed) fixed >> ZBAR_FIXED, \
fixed & ((1 << ZBAR_FIXED) - 1)
...
void dump_avg(const zbar_scanner_t *scn){
    dbprintf(1, "====\n");
    for(int i =0; i < 3; i++){
        dbprintf(1, " y0i[%d]=%d\n",
                 i, scn->y0[i & 3]);
                                ...
                                int y)
{
    ...
    dbprintf(1, " y=%d\n", y);
    /*dbprintf(1, " y0x[%d]=" PR_FIXED_FMT "\n",
             x & 3, FIXED_TO_FLOAT(scn->y0[x & 3]));
